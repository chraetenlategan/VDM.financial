// ChurchImporter.js — SecInfo import logic for Church entities

class ChurchImporter {
  static extract(cell, fmtAddr, setField, em) {
    const entityName = cell('H8') || '';
    setField('companyName', entityName);

    // Registration number is optional for churches
    ...
    ChurchImporter._mapSigner(cell('AA10'), setField);

    const ecCard = document.getElementById('ec-church');
    if (ecCard) em.selectEntity(ecCard, 'church');

    // ── Church Council Members (optional) ──
    document.getElementById('directors-list').innerHTML = '';
    em.directors = [];
    em.directorCount = 0;

    // Search for various header names the section might use
    let headerRow = 0;
    for (let r = 40; r <= 70; r++) {
      const val = cell('D' + r);
      if (val && /^(COUNCIL\s*MEMBERS?|DIRECTORS|TRUSTEES|KERKRAAD)$/i.test(val.trim())) {
        headerRow = r;
        break;
      }
    }
    ...
    const imported = [];
    if (entityName) imported.push('church name');
    ...
    if (membersImported > 0) imported.push(membersImported + ' council member(s)');
    return imported;
  }
